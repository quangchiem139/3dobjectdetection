...
int main(int argc, char** argv) 
{
  ...
  pcl::RangeImage::extractFarRanges(scene_cloud2,scene_far_ranges); 
	...
	//// Select keypoint descriptor manually.
	...
	for (size_t i = 0; i < 1; ++i)
	{
		...
		{
			//cout << "Finding match at scene point: " << scene_narf_descriptors_ptr->at(i) << std::endl;
			for (size_t j = 0; j < model_narf_descriptors_ptr->size(); ++j) 
			{
					...
					float neigh_sqr_dists = pcl::L1_Norm(scene_narf_descriptors_ptr->at(tmp).descriptor,model_narf_descriptors_ptr->at(j).descriptor,36);
					neigh_sqr_dists = neigh_sqr_dists / 36;
					if(neigh_sqr_dists < max_corr_dist_ && neigh_sqr_dists > min_corr_dist_ )
					{
						//pcl::Correspondence corr(static_cast<int> (j), static_cast<int> (i), neigh_sqr_dists);
						pcl::Correspondence corr(static_cast<int> (j), tmp, neigh_sqr_dists);
						model_scene_corrs->push_back(corr);
						//std::cout << "neigh_sqr_dists:: "<< neigh_sqr_dists << std::endl;
						//std::cout << "(k_indices,index,k_distances): " << "(" << j << "," << i << "," << neigh_sqr_dists << ")" << std::endl;
						std::cout << "(k_indices,index,k_distances): " << "(" << j << "," << tmp << "," << neigh_sqr_dists << ")" << std::endl;
					}
					
			}
			//int found_neighs = match_search.nearestKSearch(scene_narf_descriptors_ptr->at(i), k, neigh_indices, neigh_sqr_dists);
